...
from evaluations import RRSE,CORR
...
def load_adj_BJ(adjtype):
    A = pd.read_csv ("/home/Zxf1/Debias_for_Traffic/Datasets/BJ_speed/arm_zhongxin.csv", index_col=0)
    A = np.array(A)
...
def load_dataset(dataset_dir, batch_size, valid_batch_size, test_batch_size,horizon, n_obs=None, fill_zeroes=True):
    data = {}
...
def calc_metrics(preds, labels, null_val=0.):
    ...
    #
    # corr = CORR (preds, labels)
    #
    # rrse = RRSE (preds, labels)

    rmse = torch.sqrt(mse)
    return mae, mape, rmse

def calc_metrics_singel(preds, labels, null_val=0.):
    if np.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
    ...
    pre_single = preds.chunk(preds.shape[1],1)
    lab_single = labels.chunk(labels.shape[1],1)
    mae_list = []
    rmse_list = []
    mape_list = []
    for road in zip(pre_single,lab_single):
        mse = (road[0]- road[1]) ** 2
        mae = torch.abs (road[0] - road[1])
        mape = mae / road[1]
        mae, mape, mse = [mask_and_fillna (l, mask) for l in [mae, mape, mse]]
        rmse = torch.sqrt (mse)
    ...
    mae_list = pd.DataFrame(mae_list).to_csv('/home/Zxf1/Debias_Reference_Code/ST-MoE_Result_record/GWN/BJ_C/moe_BJ_C_mae_h3.csv')
    mape_list = pd.DataFrame(mape_list).to_csv('/home/Zxf1/Debias_Reference_Code/ST-MoE_Result_record/GWN/BJ_C/moe_BJ_C_mape_h3.csv')
    rmse_list = pd.DataFrame(rmse_list).to_csv('/home/Zxf1/Debias_Reference_Code/ST-MoE_Result_record/GWN/BJ_C/moe_BJ_C_rmse_h3.csv')
...
def out_calc_metrics(preds, labels, null_val=0.):
    if np.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
...
def calc_tstep_metrics(model, device, test_loader, scaler, realy, seq_length) -> pd.DataFrame:
    ...
    for i in range(seq_length):
        ...
        real = realy[:, :, i]
        test_singel.append([x.item() for x in calc_metrics_singel(pred, real)])
...
def calc_tstep_metrics_moe(adj_mx,model, device, test_loader, scaler, realy, seq_length) -> pd.DataFrame:
    model.eval()
    outputs = []
    for _, (x, __) in enumerate(test_loader.get_iterator()):
        testx = torch.Tensor(x).to(device).transpose(1, 3)
        ...
        with torch.no_grad():
            preds , aux_loss = model(adj_mx,testx)
...
def make_graph_inputs(args, device):
    ...
    #BJ
    adj_mx = load_adj_BJ ( args.adjtype)
...
def get_shared_arg_parser():
    ...
    parser.add_argument('--data', type=str, default='data/BJ_center', help='data path')
    ...
    parser.add_argument('--do_graph_conv',default=True, action='store_true',
    ...
    parser.add_argument('--randomadj',default=True, action='store_true',
    ...
    parser.add_argument('--num_nodes', type=int, default=142, help='number of nodes')
    ...
    parser.add_argument('--cat_feat_gc',default=False, action='store_true')
    parser.add_argument('--fill_zeroes',default=True, action='store_true')
