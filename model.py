...
class GWNet(nn.Module):
    def __init__(self, device, num_nodes,out_dim, dropout=0.3, supports=None, do_graph_conv=True,
                 addaptadj=True, aptinit=None, in_dim=2,
    ...
    def forward(self, x):
        ...
        # roadlevel = torch.tensor (np.load ('/home/Zxf1/Debias_for_Traffic/Datasets/BJ_speed/BJ_center_roadlevel.npy',
        #                                    allow_pickle=True).astype (float)).to ('cuda:1').float ()
        # roadlevel = roadlevel.unsqueeze(0)
        # roadlevel = roadlevel.repeat(13,1,1).permute(1,2,0)
        # x = x+roadlevel*0.1
        in_len = x.size(3)
