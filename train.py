...
def main(args, **model_kwargs):
    ...
    metrics = []
    best_model_save_path = os.path.join(args.save, 'BJ_best_model_%s.pth' %args.horizon)
    ...
    for _ in mb:
        ...
        print ('mae:', np.mean(valid_loss), 'mape:', np.mean(train_mape), 'rmse:', np.mean(valid_rmse))
        ...
        met_df.round(6).to_csv(f'{args.save}/metrics%s.csv' %args.horizon)
    ...
    test_met_df, yhat = calc_tstep_metrics(engine.model, device, data['test_loader'], scaler, realy, args.horizon)
    test_met_df.round(6).to_csv(os.path.join(args.save, 'test_metric%s.csv' %args.horizon))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', type=str, default='logs/baseline_BJ', help='save path')
