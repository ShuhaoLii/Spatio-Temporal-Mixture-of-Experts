import torch
import numpy as np
import pandas as pd
import time
import util
from moe_engine import Trainer
import os
from durbango import pickle_save
from fastprogress import progress_bar

from moe import MoE
from util import calc_tstep_metrics_moe
from exp_results import summary


def main(args, **model_kwargs):
    device = torch.device(args.device)
    data = util.load_dataset(args.data, args.batch_size, args.batch_size, args.batch_size,args.horizon, n_obs=args.n_obs, fill_zeroes=args.fill_zeroes)
    scaler = data['scaler']
    aptinit, supports = util.make_graph_inputs(args, device)
    num_nodes = args.num_nodes
    num_features =args.in_dim
    num_timesteps_input = args.seq_length
    num_experts = args.experts

    #LA
    # adj_mx = np.load ('/home/Zxf1/Debias_Reference_Code/Baseline_model/ASTGCN/data/METR_LA/adj_mat.npy')

    #BJ
    A = pd.read_csv ("/home/Zxf1/Debias_for_Traffic/Datasets/BJ_speed/arm_zhongxin.csv", index_col=0)
    A = np.array(A)
    adj_mx = A.astype(np.float32)

    adj_mx = torch.from_numpy (adj_mx).float ().to (device)

    model = MoE(args, device, supports, aptinit,num_nodes, num_features, num_timesteps_input, num_experts, **model_kwargs)
    if args.checkpoint:
        model.load_checkpoint(torch.load(args.checkpoint))
    model.to(device)
    engine = Trainer.from_args(model, scaler, args)
    metrics = []
    best_model_save_path = os.path.join(args.save, '%smoe_best_model_%s.pth' %(num_experts,args.horizon))
    lowest_mae_yet = 100  # high value, will get overwritten
    mb = progress_bar(list(range(1, args.epochs + 1)))
    epochs_since_best_mae = 0
    for _ in mb:
        train_loss, train_mape, train_rmse = [], [], []
        data['train_loader'].shuffle()
        for iter, (x, y) in enumerate(data['train_loader'].get_iterator()):
            trainx = torch.Tensor(x).to(device).transpose(1, 3)
            trainy = torch.Tensor(y).to(device).transpose(1, 3)
            yspeed = trainy[:, 0, :, :]
            if yspeed.max() == 0: continue
            mae, mape, rmse = engine.train(adj_mx,trainx, yspeed)
            train_loss.append(mae)
            train_mape.append(mape)
            train_rmse.append(rmse)
            if args.n_iters is not None and iter >= args.n_iters:
                break
        engine.scheduler.step()
        _, valid_loss, valid_mape, valid_rmse = eval_(adj_mx,data['val_loader'], device, engine)
        print('mae: ',np.mean(valid_loss),'mape: ',np.mean(valid_mape),'rmse: ',np.mean(valid_rmse))
        m = dict(train_loss=np.mean(train_loss), train_mape=np.mean(train_mape),
                 train_rmse=np.mean(train_rmse), valid_loss=np.mean(valid_loss),
                 valid_mape=np.mean(valid_mape), valid_rmse=np.mean(valid_rmse))

        m = pd.Series(m)
        metrics.append(m)
        if m.valid_loss < lowest_mae_yet:
            torch.save(engine.model.state_dict(), best_model_save_path)
            lowest_mae_yet = m.valid_loss
            epochs_since_best_mae = 0
        else:
            epochs_since_best_mae += 1
        met_df = pd.DataFrame(metrics)
        mb.comment = f'best val_loss: {met_df.valid_loss.min(): .3f}, current val_loss: {m.valid_loss:.3f}, current train loss: {m.train_loss: .3f}'
        met_df.round(6).to_csv(f'{args.save}/%smoe_metrics%s.csv' %(num_experts,args.horizon))
        if epochs_since_best_mae >= args.es_patience: break
    # Metrics on test data
    engine.model.load_state_dict(torch.load(best_model_save_path))
    realy = torch.Tensor(data['y_test']).transpose(1, 3)[:, 0, :, :].to(device)
    test_met_df, yhat = calc_tstep_metrics_moe(adj_mx,engine.model, device, data['test_loader'], scaler, realy, args.horizon)
    print(test_met_df)
    test_met_df.round(6).to_csv(os.path.join(args.save, '%smoe_test_metric%s.csv' %(num_experts,args.horizon)))
    print(summary(args.save))

def eval_(adj_mx,ds, device, engine):
    """Run validation."""
    valid_loss = []
    valid_mape = []
    valid_rmse = []
    s1 = time.time()
    for (x, y) in ds.get_iterator():
        testx = torch.Tensor(x).to(device).transpose(1, 3)
        testy = torch.Tensor(y).to(device).transpose(1, 3)
        metrics = engine.eval(adj_mx,testx, testy[:, 0, :, :])
        valid_loss.append(metrics[0])
        valid_mape.append(metrics[1])
        valid_rmse.append(metrics[2])
    total_time = time.time() - s1
    return total_time, valid_loss, valid_mape, valid_rmse


if __name__ == "__main__":
    parser = util.get_shared_arg_parser()
    parser.add_argument ('--experts', type=int, default=4, help='')
    # parser.add_argument ('--horizon', type=int, default=12, help='')
    parser.add_argument('--epochs', type=int, default=50, help='')
    parser.add_argument('--clip', type=int, default=3, help='Gradient Clipping')
    parser.add_argument('--weight_decay', type=float, default=0.0001, help='weight decay rate')
    parser.add_argument('--learning_rate', type=float, default=0.001, help='learning rate')
    parser.add_argument('--lr_decay_rate', type=float, default=0.97, help='learning rate')
    parser.add_argument('--save', type=str, default='logs/moe_BJ', help='save path')
    parser.add_argument('--n_iters', default=None, help='quit after this many iterations')
    parser.add_argument('--es_patience', type=int, default=20, help='quit if no improvement after this many iterations')

    args = parser.parse_args()
    t1 = time.time()
    if not os.path.exists(args.save):
        os.mkdir(args.save)
    pickle_save(args, f'{args.save}/args_%s.pkl' %args.horizon)
    main(args)
    t2 = time.time()
    mins = (t2 - t1) / 60
    print(f"Total time spent: {mins:.2f} seconds")
